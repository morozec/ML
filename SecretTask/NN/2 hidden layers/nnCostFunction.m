...
for i=1:m,
  ...
  for k=1:K,
    J=J-y_vector(k)*log(h3(i,k))-(1-y_vector(k))*log(1-h3(i,k));
  end
end
J=J/m;
...
Delta3 = zeros(size(Theta3,1),size(Theta3,2)); %K x (s2+1)

Theta1NoBias= Theta1(:,2:end);  %s2 x n
Theta2NoBias= Theta2(:,2:end);  %K x s2
Theta3NoBias= Theta3(:,2:end);  %K x s2
...
for t = 1:m,
  ...
  for k=1:K,
    delta4(k)=a4(k)-y_vector(k);
  end
   
  delta3=(Theta3NoBias')*delta4.* sigmoidGradient(z3);
...
Theta2ZeroBias=[zeros(s3,1) Theta2NoBias];  %K x (s2+1)
Theta3ZeroBias=[zeros(K,1) Theta3NoBias];

Theta1_grad = Delta1/m + lambda/m*Theta1ZeroBias;
Theta2_grad = Delta2/m + lambda/m*Theta2ZeroBias;
Theta3_grad = Delta3/m + lambda/m*Theta3ZeroBias;
